import type { SurveyConfig } from "./surveys";

export const brandFormSurvey: SurveyConfig = {
  id: "branding",
  label: "افزودن اطلاعات برند",
  title: "افزودن اطلاعات برند",
  description: "",
  orderSku: "branding",
  steps: [
    { id: 1,
      backendKey: "brand_name", question: "نام برند شما چیست؟", type: "text" },
    {
      id: 2,
      backendKey: "website_url",

      question: "آدرس سایت خود را وارد کنید.",
      type: "url",
      placeholder: "https://example.com",
    },
    {
      id: 3,
      backendKey: "brand_establishment",

      question: "شرکت شما در چه تاریخی تاسیس شده‌است؟",
      type: "shamsiDate",
      placeholder: "تاریخ تاسیس را انتخاب کنید",
    },
    {
      id: 4,
      backendKey: "employee_count",

      question: "انداز فعلی سازمان/تیم شما چقدر است ؟",
      type: "radio",
      options: [
        "میکروبیزنس (۱–۵ نفر)",
        "کسب‌وکار متوسط (۲۱–۱۰۰ نفر)",
        "سازمان بزرگ (۱۰۱–۵۰۰ نفر)",
        "سازمان Enterprise (بیش از ۵۰۰ نفر)",
      ],
    },
    {
      id: 5,
      backendKey: "business_category",

      question: "حوزه اصلی کسب‌وکار شما چیست؟",
      type: "checkbox",
      checkboxMaxSelections: 5,
      options: [
        "زیبایی، آرایشی و مراقبت شخصی",
        "مد و پوشاک",
        "کیف، کفش و اکسسوری",
        "جواهرات، طلا و کالاهای لوکس",
        "محصولات مادر، نوزاد و کودک",
        "اسباب‌بازی، بازی فیزیکی و سرگرمی کودک",
        "حیوانات خانگی، محصولات پت و خدمات دامپزشکی",
        "غذا و نوشیدنی",
        "رستوران، کافه، فست‌فود و غذای بیرون‌بر",
        "هتل، اقامتگاه و مهمان‌داری",
        "تجهیزات سفر و کمپینگ",
        "عمده‌فروشی، پخش و تامین کالا",
        "واردات، صادرات و بازرگانی بین‌الملل",
        "دکوراسیون داخلی و مبلمان",
        "لوازم خانگی و آشپزخانه",
        "لوازم ساختمانی و تجهیزات مصرفی ساختمان",
        "گل، گیاه، باغبانی و فضای سبز خانگی",
        "ابزارآلات و لوازم الکتریکی",
        "موبایل، تبلت و لوازم جانبی",
        "کامپیوتر، لپ‌تاپ و تجهیزات جانبی",
        "سخت‌افزار، شبکه و تجهیزات IT",
        "خدمات فناوری اطلاعات و پشتیبانی فنی",
        "مخابرات، اینترنت و ارتباطات",
        "امنیت سایبری، حریم خصوصی و امنیت داده",
        "داده، هوش مصنوعی، تحلیل و اتوماسیون",
        "رسانه، نشر و پلتفرم‌های محتوایی",
        "تولید محتوا و خدمات محتوایی",
        "تبلیغات، بازاریابی و رسانه",
        "روابط عمومی و ارتباطات برند",
        "طراحی گرافیک، برندینگ و هویت بصری",
        "استودیوهای خلاق و خدمات طراحی",
        "عکاسی، فیلم‌برداری و تولید ویدئو",
        "چاپ، بسته‌بندی و خدمات چاپی",
        "هنر، صنایع خلاق و محصولات فرهنگی",
        "موسیقی، سینما و سرگرمی",
        "بازی و سرگرمی دیجیتال",
        "رویدادها، نمایشگاه‌ها و کنفرانس‌ها",
        "فروش بلیت و خدمات ورودی",
        "آموزش مدرسه‌ای و آکادمیک",
        "آموزش آزاد و مهارت‌آموزی",
        "آموزش آنلاین و پلتفرم‌های یادگیری",
        "کوچینگ، منتورینگ و آموزش حرفه‌ای",
        "کتاب، نشر، لوازم تحریر و کمک‌آموزشی",
        "پژوهش، علم، آزمایشگاه و نوآوری",
        "پزشکی، درمان و خدمات کلینیکی",
        "دندانپزشکی و خدمات دهان و دندان",
        "داروخانه و توزیع دارویی",
        "داروسازی و بیوتکنولوژی",
        "تجهیزات پزشکی و ملزومات درمانی",
        "آزمایشگاه، تشخیص و خدمات پاراکلینیکی",
        "سلامت، تندرستی و سبک زندگی سالم",
        "تناسب اندام، باشگاه و تجهیزات ورزشی",
        "روانشناسی، مشاوره فردی و سلامت روان",
        "بانکداری، پرداخت و خدمات پولی",
        "حسابداری، مالیات و حسابرسی",
        "خدمات مالی و مدیریت مالی",
        "بیمه و خدمات بیمه‌ای",
        "سرمایه‌گذاری، بازار سرمایه و مدیریت دارایی",
        "تامین مالی، اعتبار و تسهیلات",
        "فین‌تک و فناوری‌های مالی",
        "رمزارز، بلاکچین و دارایی دیجیتال",
        "خدمات حقوقی، وکالت و مشاوره حقوقی",
        "امور ثبتی، قراردادی و مالکیت فکری",
        "مشاوره کسب‌وکار و مدیریت",
        "خدمات حرفه‌ای و برون‌سپاری تخصصی",
        "منابع انسانی، استخدام و کاریابی",
        "آموزش سازمانی و توسعه سرمایه انسانی",
        "فضای کار اشتراکی و خدمات اداری",
        "خدمات پشتیبانی کسب‌وکار و عملیات اداری",
        "املاک، خرید، فروش و اجاره",
        "مدیریت املاک و مستغلات",
        "ساخت‌وساز، پیمانکاری و اجرای پروژه",
        "توسعه املاک و پروژه‌های عمرانی",
        "معماری، طراحی داخلی و طراحی فضا",
        "تاسیسات، تعمیرات و نگهداری ساختمان",
        "خدمات فنی، برق، مکانیک و تاسیسات ساختمانی",
        "نظافت، خدمات ساختمانی و نگهداری اماکن",
        "تولید، کارخانه و صنایع تولیدی",
        "ماشین‌آلات و تجهیزات صنعتی",
        "ابزارآلات و تجهیزات فنی",
        "اتوماسیون صنعتی و کنترل",
        "مواد اولیه، قطعات و ملزومات تولید",
        "بسته‌بندی صنعتی و خدمات وابسته",
        "چاپ صنعتی و تولید سفارشی",
        "بازرسی فنی، کنترل کیفیت و خدمات مهندسی",
        "انرژی، نفت، گاز و پتروشیمی",
        "برق، آب، گاز و خدمات شهری",
        "انرژی‌های تجدیدپذیر و فناوری پاک",
        "محیط‌زیست، پایداری و مدیریت سبز",
        "بازیافت، پسماند و مدیریت منابع",
        "کشاورزی، زراعت و باغداری",
        "دامپروری، طیور و شیلات",
        "صنایع غذایی اولیه و فرآوری کشاورزی",
        "فناوری کشاورزی و تجهیزات کشاورزی",
        "خودرو، قطعات و لوازم یدکی",
        "خدمات خودرویی، تعمیر، سرویس و نگهداری",
        "موتورسیکلت، دوچرخه و حمل‌ونقل سبک",
        "حمل‌ونقل مسافر",
        "حمل‌ونقل کالا و باربری",
        "لجستیک، انبارداری و زنجیره تامین",
        "خدمات پیک، ارسال و تحویل کالا",
        "ورزش، تفریح و سرگرمی حضوری",
        "باشگاه‌ها، مراکز ورزشی و آکادمی‌ها",
        "گردشگری تفریحی و تجربه‌محور",
        "خدمات شخصی و سبک زندگی",
        "خدمات محلی و خدمات در محل",
        "خشکشویی، نظافت و خدمات خانگی",
        "امنیت، حفاظت و سیستم‌های ایمنی",
        "دولت و نهادهای عمومی",
        "سازمان‌های غیرانتفاعی، خیریه و اجتماعی",
        "انجمن‌ها، اتحادیه‌ها و نهادهای صنفی",
        "خدمات فرهنگی، اجتماعی و جامعه‌محور",
      ],
    },
    {
      id: 6,
      backendKey: "business_communication_model",

      question: "مدل ارتباط تجاری کسب‌وکار با مشتریان چیست؟",
      type: "checkbox",
      options: [
        "B2C (Business to Consumer)",
        "B2B (Business to Business)",
        "B2B2C (Business to Business to Consumer)",
        "C2C (Consumer to Consumer)",
        "C2B (Consumer to Business)",
        "B2G (Business to Government)",
        "C2G (Consumer to Government)",
        "G2C (Government to Consumer)",
        "G2B (Government to Business)",
        "B2E (Business to Employee)",
        "P2P (Peer to Peer)",
        "G2G (Government to Government)",
      ],
    },
    {
      id: 7,
      backendKey: "business_revenue_model",

      question: "مدل درآمدی کسب‌وکار شما کدام است ؟",
      type: "checkbox",
      options: [
        "فروش محصول (Product Sales)",
        "دریافت هزینه خدمات (Service Fees)",
        "اشتراک دوره‌ای، ماهانه یا سالانه (Subscription)",
        "کمیسیون از معامله (Commission)",
        "کارمزد تراکنش (Transaction Fee)",
        "درآمد از تبلیغات (Advertising)",
        "نسخه رایگان + ارتقاء پولی (Freemium)",
        "لایسنس / حق استفاده (Licensing / Royalties)",
        "اجاره یا لیزینگ (Rental / Leasing)",
        "درآمد از همکاری در فروش (Affiliate / Referral Fees)",
        "درآمدزایی از داده، گزارش، بینش آماری یا API داده (Data Monetization)",
        "اسپانسرشیپ (Sponsorship)",
        "پرداخت بر اساس مصرف (Usage / Pay-as-you-go)",
        "کارمزد واسطه‌گری (Brokerage Fees)",
        "حق عضویت (Membership Fees)",
        "هزینه راه‌اندازی، نصب یا پیاده‌سازی (Setup / Implementation Fee)",
        "هزینه نگهداری و پشتیبانی (Maintenance / Support Fees)",
        "فروش بلیت / هزینه ورودی (Ticketing / Admission Fees)",
        "کمک مالی / گرنت / حمایت بلاعوض (Donations / Grants)",
        "درآمد بهره، سود تسهیلات یا تامین مالی (Interest / Financing Income)",
        "حق فرانچایز / اعطای نمایندگی (Franchise Fees)",
      ],
    },
    {
      id: 8,
      backendKey: "purchase_channels",

      question: "مشتریان از چه طریقی از شما خرید می‌کنند؟",
      type: "radio",
      options: ["آنلاین", "آفلاین", "هر دو"],
    },
    {
      id: 9,
      backendKey: "online_purchase_channels",

      question: "کدام روش های فروش آنلاین براش شما فعال هستند",
      type: "checkbox",
      options: ["وبسایت", "سوشال مدیا", "اپلیکیشن"],
      showIf: {
        parentQuestion: "مشتریان از چه طریقی از شما خرید می‌کنند؟",
        equals: "آنلاین",
      },
    },
    {
      id: 10,
      backendKey: "offline_purchase_channels",

      question: "کدام روش های فروش آفلاین براش شما فعال هستند",
      type: "checkbox",
      options: [
        "فروشگاه حضوری",
        "فروش تلفنی",
        "تیم فروش/ ویزیتور",
        "نمایندگی",
        "عمده فروشی",
        "پخش و توضیع",
        "فروش در محل مشتری",
      ],
      showIf: {
        parentQuestion: "مشتریان از چه طریقی از شما خرید می‌کنند؟",
        equals: "آفلاین",
      },
    },
    {
      id: 11,
      backendKey: "both_purchase_channels",

      question: "کدام روش های فروش براش شما فعال هستند",
      type: "checkbox",
      options: [
        "فروشگاه حضوری",
        "فروش تلفنی",
        "تیم فروش/ ویزیتور",
        "نمایندگی",
        "عمده فروشی",
        "پخش و توضیع",
        "فروش در محل مشتری",
        "وبسایت",
        "سوشال مدیا",
        "اپلیکیشن",
      ],
      showIf: {
        parentQuestion: "مشتریان از چه طریقی از شما خرید می‌کنند؟",
        equals: "هر دو",
      },
    },
    {
      id: 12,
      backendKey: "top_sales_channel",

      question: "در حال حاضر بیشترین فروش از کدام کانال است؟",
      type: "radio",
      options: [
        "وب‌سایت",
        "اپلیکیشن",
        "شبکه‌های اجتماعی",
        "مارکت‌پلیس‌ها (مثل Amazon، دیجی‌کالا و …)",
        "جستجو و ورودی ارگانیک (Search / SEO)",
        "تبلیغات آنلاین (انواع ادز گوگل، متا، یکتانت...)",
        "ایمیل مارکتینگ",
        "پیامک",
        "پیام مستقیم(بله، روبیکا، واتساپ،‌چت سایت و ...)",
      ],
    },
    {
      id: 13,
      backendKey: "best_selling_product",

      question:
        "پرفروش‌ترین محصول یا خدمت شما کدام است؟ نام ببرید یا لینک محصول را قرار دهید.",
      type: "textarea",
    },
    {
      id: 14,
      backendKey: "most_profitable_product",

      question:
        "سودآورترین محصول یا خدمت شما چیست؟ نام ببرید یا لینک محصول را قرار دهید.",
      type: "textarea",
    },
    {
      id: 15,
      backendKey: "peak_sales_periods",

      question:
        "مهم‌ترین زمان فروش برای کسب‌و‌کار شما در سال کدام یک از موقعیت‌های زیر است؟.",
      otherOption: "سایر",
      otherPlaceholder: "موضوع مورد نظر خود را بنویسید",
      type: "checkbox",
      options: [
        "نوروز",
        "سیزده به در",
        "بلک فرایدی",
        "شب یلدا",
        "ماه رمضان",
        "ولنتاین",
        "روز مادر",
        "روز پدر",
        "بازگشایی مدارس",
        "عید فطر",
        "عید غدیر",
        "روز معلم",
        "روز پرستار",
        "سایر",
      ],
    },
    {
      id: 16,
      backendKey: "user_journey",

      question:
        "مسیر خرید مشتری شما چیست؟ (معمولاً از چه مسیری با برند شما آشنا می‌شود و به خرید می‌رسد؟)",
      type: "repeater",
      repeaterVariant: "journeyFunnel",
      repeaterMinRows: 3,
      repeaterFields: [
        {
          key: "title",
          label: "عنوان مرحله",
          type: "text",
          placeholder: "مثلاً آشنایی",
        },
        {
          key: "description",
          label: "توضیح مرحله",
          type: "text",
          placeholder: "مثلاً آشنایی با محصول از طریق سرچ گوگل",
        },
      ],
    },
    {
      id: 17,
      backendKey: "business_marketing_channels",

      question: "کانال‌های مارکتینگی خود را مشخص کنید.",
      type: "checkbox",
      options: [
        "اینستاگرام",
        "تلگرام",
        "لینکدین",
        "یوتیوب",
        "آپارات",
        "نماشا",
        "تیک‌تاک",
        "ایکس",
        "فیسبوک",
        "پینترست",
        "روبیکا",
        "بله",
        "ایتا",
        "سروش پلاس",
        "فیلیمو",
        "نماوا",
        "گوگل ادز",
        "گوگل دیسپلی",
        "یکتانت",
        "تپسل",
        "دیما",
        "کافه بازار ادز",
        "اسنپ اد",
        "تپسی اد",
        "دیجی کالا ادز",
        "دیوار ادز",
        "Amazon Ads",
        "eBay Ads",
        "تبلیغات بنری در سایت‌ها",
        "ایمیل",
        "پیامک",
        "نوتیف اپ",
        "پوش نوتیف وب",
        "اینفلوئنسر مارکتینگ",
        "سفیر برند",
        "اسپانسرینگ",
        "رپورتاژ",
        "افیلیت مارکتینگ",
        "ترب",
        "اسنپ مارکت",
        "باما",
        "شیپور",
        "نقشه نشان",
        "نقشه بلد",
        "اسپاتیفای",
        "اپل پادکست",
        "ایسمینار",
        "اسکای روم",
        "بیلبورد",
        "تلویزیون",
        "رادیو",
        "مترو و اتوبوس",
        "تبلیغات روی لباس یا وسایل نقلیه",
        "نمایشگاه‌ها و رویدادها",
        "سایر تبلیغات محیطی",
      ],
    },
    {
      id: 18,
      backendKey: "business_problem_solved",

      question:
        "جمله زیر را تکمیل کنید: «من با کسب‌وکارم به افراد کمک می‌کنم تا …»",
      type: "textarea",
    },
    {
      id: 19,
      backendKey: "brand_name_philosophy",

      question: "دلیل و فلسفه انتخاب نام برند شما چه بوده‌است؟",
      type: "textarea",
    },
    {
      id: 20,
      backendKey: "brand_story",

      question: "داستان شکل‌گیری برند خود را شرح دهید.",
      type: "textarea",
    },
    {
      id: 21,
      backendKey: "visual_identity",

      question:
        "آیا هویت بصری مکتوب شامل لوگو، رنگ‌ها و فونت دارید؟ لطفاً هر یک را به‌صورت جداگانه بارگذاری کنید.",
      type: "brandVisualIdentity",
    },
    {
      id: 22,
      backendKey: "brand_tagline",

      question: "شعار برند خود را در صورت وجود، بنویسید.",
      type: "text",
      isAllowedEmpty: true,
    },
    {
      id: 23,
      backendKey: "brand_archetype",

      question: "آرکی‌تایپ یا شخصیت اصلی برند شما چیست؟",
      type: "percentageAllocation",
      options: [
        "1. معصوم | Innocent",
        "این شخصیت حس پاکی، امید و امنیت می‌دهد.",
        "2. کاوشگر | Explorer",
        "این شخصیت دوست دارد آزاد باشد و دنیا را کشف کند.",
        "3. فرزانه | Sage",
        "این شخصیت با دانایی و فکر دقیق، راه درست را نشان می‌دهد.",
        "4. قهرمان | Hero",
        "این شخصیت با شجاعت تلاش می‌کند تا امید را در دل دیگران رنده کند.",
        "5. یاغی | Outlaw / Rebel",
        "این شخصیت از قوانین معمول عبور کرده و راهی تازه می‌سازد.",
        "6. جادوگر | Magician",
        "این شخصیت با زاویه دید خاص و خلاقانه، تغییرات بزرگی می‌آفریند.",
        "7. آدم معمولی | Everyman",
        "این شخصیت ساده، صمیمی و مثل عموم مردم است.",
        "8. عاشق | Lover",
        "این شخصیت بسیار احساسی، مهربان است و شخصیتی جذاب و گیرا دارد.",
        "9. دلقک | Jester",
        "این شخصیت با شوخی و بازی، سعی دارد حال همه را خوب کند.",
        "10. مراقب | Caregiver",
        "این شخصیت مهربان است و از دیگران با دل‌گرمی حمایت می‌کند.",
        "11. خالق | Creator",
        "این شخصیت با استفاده از قوه تخیل، چیزهای تازه و نوآورانه می‌سازد.",
        "12. حاکم | Ruler",
        "این شخصیت منظم و قدرتمند است و همه‌چیز را به خوبی مدیریت می‌کند.",
      ],
    },
    {
      id: 24,
      backendKey: "business_redlines",

      question:
        "چه موضوعاتی را نمی‌خواهید به هیچ عنوان در محتوای شما، چه متنی و چه تصویری، مطرح شوند؟",
      type: "checkbox",
      otherOption: "سایر",
      otherPlaceholder: "موضوع مورد نظر خود را بنویسید",
      options: [
        "بزرگ‌نمایی یا ارائه اطلاعات گمراه‌کننده درباره محصول/خدمت",
        "ادعاهای تضمینی، قطعی یا بدون ریسک",
        "ادعاهای بدون مدرک درباره کیفیت، نتیجه یا اثرگذاری",
        "محتوای سیاسی",
        "بهره‌برداری تبلیغاتی از مفاهیم، مناسبت‌ها یا نمادهای مذهبی در محتوا",
        "محتوای جنسی، تحریک‌آمیز یا دوپهلو",
        "محتوای غیرقانونی یا نیازمند مجوز",
        "مقایسه تخریبی با رقبا",
        "استفاده از ترس، فشار یا اضطرار ساختگی برای فروش",
        "هیچ خط‌قرمز خاصی نداریم",
        "سایر",
      ],
    },
    {
      id: 25,
      backendKey: "key_brand_events",

      question: "رویدادها و مناسبت‌های کلیدی برند شما کدام‌اند؟",
      type: "namedShamsiDates",
      placeholder: "مثال: سالگرد تأسیس، جشنواره فروش",
    },
    {
      id: 26,
      backendKey: "target_audience",

      question: "مصرف کننده نهایی محصول / خدمت شما چه کسی است؟",
      type: "radio",
      options: ["خود خریدار", "شخص دیگری"],
    },
    {
      id: 27,
      backendKey: "end_user_is_buyer",

      question: "پرسونای خریدار/مصرف کننده شما چیست؟",
      type: "personaFields",
      showIf: {
        parentQuestion: "مصرف کننده نهایی محصول / خدمت شما چه کسی است؟",
        equals: "خود خریدار",
      },
    },
    {
      id: 28,
      backendKey: "end_user_is_anotherperson",

      question: "پرسونای مخاطب محتوا/تبلیغات شما چیست؟",
      type: "personaFields",
      showIf: {
        parentQuestion: "مصرف کننده نهایی محصول / خدمت شما چه کسی است؟",
        equals: "شخص دیگری",
      },
    },
    {
      id: 29,
      backendKey: "competitive_advantage",

      question: "مزیت‌های رقابتی اصلی شما در بازار کدام‌اند؟",
      type: "textarea",
      options: ["مثال راهنما :‌گارانتی،"],
    },
    {
      id: 30,
      backendKey: "brand_competitors",

      question: "رقبای اصلی شما کدام برندها هستند؟ لطفاً نام ببرید.",
      type: "textarea",
    },
    {
      id: 31,
      backendKey: "positive_customer_feedback",

      question:
        "در ۶ ماه اخیر، چه بازخوردهای مثبتی درباره‌ی برند، خدمات یا محصولات خود از مشتریان‌تان در پلتفرم‌های مختلف دریافت کرده‌اید؟",
      type: "textarea",
    },
    {
      id: 32,
      backendKey: "negative_customer_feedback",

      question:
        "در ۶ ماه اخیر، چه بازخوردهای منفی درباره‌ی برند، خدمات یا محصولات خود از مشتریان‌تان در پلتفرم‌های مختلف دریافت کرده‌اید؟",
      type: "textarea",
    },
    {
      id: 33,
      backendKey: "business_market_share",

      question:
        "به نظر شما، در حال حاضر چند درصد از سهم بازارِ حوزه‌ی کسب‌وکارتان را در اختیار دارید؟",
      type: "number",
      numberSuffix: "%",
      numberMin: 0,
      numberMax: 100,
      placeholder: "0",
    },
    {
      id: 34,
      backendKey: "business_target_market_share",

      question:
        "فکر می‌کنید در آینده چند درصدِ دیگر از سهم بازار را به دست خواهید آورد؟",
      type: "number",
      numberSuffix: "%",
      numberMin: 0,
      numberMax: 100,
      placeholder: "0",
    },
    {
      id: 35,
      backendKey: "business_potential_crisis_solution",

      question:
        "مهم‌ترین بحران اجتماعی، سیاسی، حتی درون سازمانی و … که ممکن است بیزینس شما را تحت تاثیر قرار دهد چیست؟ در چنین شرایطی چه راهکاری دارید؟",
      type: "textarea",
    },
    {
      id: 36,
      backendKey: "main_business_challenge",

      question:
        "اصلی‌ترین چالشی که در حال حاضر بیزینس شما با آن دست‌وپنجه نرم می‌کند چیست؟ آیا برای این چالش به راه‌حلی رسیده‌اید؟",
      type: "textarea",
    },
    {
      id: 37,
      backendKey: "main_business_goal",

      question:
        "در حال حاضر، مهم‌ترین هدفی که برای رشد کسب‌وکار خود دارید چیست؟",
      type: "checkbox",
      options: [
        "Increase Revenue افزایش درآمد",
        "Improve Profitability افزایش سودآوری",
        "Acquire New Customers / Users جذب مشتریان یا کاربران جدید",
        "Improve Customer / User Retention افزایش حفظ مشتریان یا کاربران فعلی",
        "Increase Customer / User Engagement افزایش تعامل مشتریان یا کاربران",
        "Improve Conversion Rate افزایش نرخ تبدیل",
        "Expand to New Markets / Segments ورود به بازارها یا بخش‌های جدید",
        "Launch or Improve Products / Services عرضه یا بهبود محصولات / خدمات",
        "Improve Brand Awareness افزایش آگاهی از برند",
        "Build Trust and Credibility افزایش اعتماد و اعتبار",
        "Improve Customer / User Experience بهبود تجربه مشتری یا کاربر",
        "Reduce Costs and Improve Efficiency کاهش هزینه‌ها و افزایش بهره‌وری",
        "Improve Marketing Performance بهبود عملکرد بازاریابی",
        "Strengthen Sales or Distribution Channels تقویت کانال‌های فروش یا توزیع",
        "Build Strategic Partnerships ایجاد همکاری‌های استراتژیک",
        "Increase Operational Scalability افزایش مقیاس‌پذیری عملیاتی",
        "Improve Product-Market Fit بهبود تناسب محصول/خدمت با بازار",
        "Secure Funding or Financial Resources جذب سرمایه یا منابع مالی",
        "Increase Market Share افزایش سهم بازار",
      ],
    },
    {
      id: 38,
      backendKey: "business_crm",

      question: "آیا از سیستم CRM استفاده می‌کنید؟",
      type: "radio",
      options: ["بله", "خیر"],
    },
    {
      id: 39,
      backendKey: "business_tools_and_platforms",

      question:
        "در حال حاضر از کدام ابزارها و پنل‌ها در کسب‌وکار خود استفاده می‌کنید؟",
      type: "checkbox",
      options: ["ابزارهای مارکتینگی", "ابزارهای تبلیغاتی"],
    },
    {
      id: 40,
      backendKey: "business_marketing_tools",

      question: "ابزارهای مارکتینگی خود را از لیست انتخاب کنید.",
      type: "checkbox",
      options: [
        "Google Analytics",
        "Google Search Console",
        "Google Tag Manager",
        "Microsoft Clarity",
        "Hotjar",
        "Tableau",
        "Power BI",
        "Metabase",
        "CRM",
        "HubSpot",
        "Salesforce",
        "Zoho CRM",
        "Pipedrive",
        "پنل فروشگاه اینترنتی / سایت",
        "پنل شبکه‌های اجتماعی",
        "فایل اکسل / Google Sheets",
        "webengage",
        "metrics",
        "n8n",
        "adtrace",
        "matomo",
        "semrush",
        "graphana",
        "looker",
        "all seo tools",
        "VWO",
        "Optimizely",
        "AppsFlyer",
        "Adjust",
        "Hootsuite",
        "Buffer",
        "Later",
        "Sprout Social",
        "Zapier",
        "Make.com",
        "Klaviyo",
        "ActiveCampaign",
        "Mailchimp",
        "Ahrefs",
        "Moz Pro",
        "Screaming Frog SEO Spider",
        "Surfer SEO",
        "Similarweb",
        "Google Keyword Planner",
        "Mixpanel",
        "PostHog",
      ],
      showIf: {
        parentQuestion:
          "در حال حاضر از کدام ابزارها و پنل‌ها در کسب‌وکار خود استفاده می‌کنید؟",
        includes: "ابزارهای مارکتینگی",
      },
    },
    {
      id: 41,
      backendKey: "business_advertising_tools",

      question: "ابزارهای تبلیغاتی خود را از لیست انتخاب کنید.",
      type: "checkbox",
      options: [
        "Google Ads",
        "Meta Ads Manager",
        "LinkedIn Campaign Manager",
        "TikTok Ads Manager",
        "X Ads",
        "یکتانت",
        "mediad",
        "صباویژن",
        "پنل تبلیغات اینفلوئنسری",
        "پنل ایمیل مارکتینگ",
        "پنل پیامک",
        "پنل نوتیفیکیشن / Push",
      ],
      showIf: {
        parentQuestion:
          "در حال حاضر از کدام ابزارها و پنل‌ها در کسب‌وکار خود استفاده می‌کنید؟",
        includes: "ابزارهای تبلیغاتی",
      },
    },
    {
      id: 42,
      backendKey: "new_business_communication_model",

      question: "آیا قصد دارید مدل تجاری جدیدی با مشتریان خود داشته باشید؟",
      type: "radio",
      options: ["بله", "خیر"],
    },
    {
      id: 43,
      backendKey: "want_new_business_communication_model",

      question: "مدل تجاری جدید مدنظر خود را مشخص کنید.",
      type: "checkbox",
      options: [
        "B2C (Business to Consumer)",
        "B2B (Business to Business)",
        "B2B2C (Business to Business to Consumer)",
        "C2C (Consumer to Consumer)",
        "C2B (Consumer to Business)",
        "B2G (Business to Government)",
        "C2G (Consumer to Government)",
        "G2C (Government to Consumer)",
        "G2B (Government to Business)",
        "B2E (Business to Employee)",
        "P2P (Peer to Peer)",
        "G2G (Government to Government)",
      ],
      showIf: {
        parentQuestion:
          "آیا قصد دارید مدل تجاری جدیدی با مشتریان خود داشته باشید؟",
        equals: "بله",
      },
    },
    {
      id: 44,
      backendKey: "expansion_plans",

      question: "آیا قصد ورود به حوزه یا حوزه‌های جدید کسب‌وکار را دارید؟",
      type: "radio",
      options: ["بله", "خیر"],
    },
    {
      id: 45,
      backendKey: "want_expansion_plans",

      question: "حوزه یا حوزه‌های مدنظر خود را مشخص کنید.",
      type: "checkbox",
      options: [
        "زیبایی، آرایشی و مراقبت شخصی",
        "مد، پوشاک و پوشیدنی‌ها",
        "کیف، کفش و اکسسوری",
        "جواهرات، ساعت و کالاهای لوکس",
        "محصولات مادر، نوزاد و کودک",
        "اسباب‌بازی، بازی فیزیکی و سرگرمی کودک",
        "حیوانات خانگی، محصولات پت و خدمات دامپزشکی",
        "غذا و نوشیدنی",
        "رستوران، کافه، فست‌فود و غذای بیرون‌بر",
        "هتل، اقامتگاه و مهمان‌داری",
        "سفر، گردشگری و خدمات مسافرتی",
        "عمده‌فروشی، پخش و تامین کالا",
        "واردات، صادرات و بازرگانی بین‌الملل",
        "دکوراسیون داخلی و مبلمان",
        "لوازم خانگی و آشپزخانه",
        "لوازم ساختمانی و تجهیزات مصرفی ساختمان",
        "گل، گیاه، باغبانی و فضای سبز خانگی",
        "ابزارآلات و لوازم الکتریکی",
        "موبایل، تبلت و لوازم جانبی",
        "کامپیوتر، لپ‌تاپ و تجهیزات جانبی",
        "سخت‌افزار، شبکه و تجهیزات IT",
        "خدمات فناوری اطلاعات و پشتیبانی فنی",
        "مخابرات، اینترنت و ارتباطات",
        "امنیت سایبری، حریم خصوصی و امنیت داده",
        "داده، هوش مصنوعی، تحلیل و اتوماسیون",
        "رسانه، نشر و پلتفرم‌های محتوایی",
        "تولید محتوا و خدمات محتوایی",
        "تبلیغات، بازاریابی و رسانه",
        "روابط عمومی و ارتباطات برند",
        "طراحی گرافیک، برندینگ و هویت بصری",
        "استودیوهای خلاق و خدمات طراحی",
        "عکاسی، فیلم‌برداری و تولید ویدئو",
        "چاپ، بسته‌بندی و خدمات چاپی",
        "هنر، صنایع خلاق و محصولات فرهنگی",
        "موسیقی، سینما و سرگرمی",
        "بازی و سرگرمی دیجیتال",
        "رویدادها، نمایشگاه‌ها و کنفرانس‌ها",
        "فروش بلیت و خدمات ورودی",
        "آموزش مدرسه‌ای و آکادمیک",
        "آموزش آزاد و مهارت‌آموزی",
        "آموزش آنلاین و پلتفرم‌های یادگیری",
        "کوچینگ، منتورینگ و آموزش حرفه‌ای",
        "کتاب، نشر، لوازم تحریر و کمک‌آموزشی",
        "پژوهش، علم، آزمایشگاه و نوآوری",
        "پزشکی، درمان و خدمات کلینیکی",
        "دندانپزشکی و خدمات دهان و دندان",
        "داروخانه و توزیع دارویی",
        "داروسازی و بیوتکنولوژی",
        "تجهیزات پزشکی و ملزومات درمانی",
        "آزمایشگاه، تشخیص و خدمات پاراکلینیکی",
        "سلامت، تندرستی و سبک زندگی سالم",
        "تناسب اندام، باشگاه و خدمات ورزشی",
        "روانشناسی، مشاوره فردی و سلامت روان",
        "بانکداری، پرداخت و خدمات پولی",
        "حسابداری، مالیات و حسابرسی",
        "خدمات مالی و مدیریت مالی",
        "بیمه و خدمات بیمه‌ای",
        "سرمایه‌گذاری، بازار سرمایه و مدیریت دارایی",
        "تامین مالی، اعتبار و تسهیلات",
        "فین‌تک و فناوری‌های مالی",
        "رمزارز، بلاکچین و دارایی دیجیتال",
        "خدمات حقوقی، وکالت و مشاوره حقوقی",
        "امور ثبتی، قراردادی و مالکیت فکری",
        "مشاوره کسب‌وکار و مدیریت",
        "خدمات حرفه‌ای و برون‌سپاری تخصصی",
        "منابع انسانی، استخدام و کاریابی",
        "آموزش سازمانی و توسعه سرمایه انسانی",
        "فضای کار اشتراکی و خدمات اداری",
        "خدمات پشتیبانی کسب‌وکار و عملیات اداری",
        "املاک، خرید، فروش و اجاره",
        "مدیریت املاک و مستغلات",
        "ساخت‌وساز، پیمانکاری و اجرای پروژه",
        "توسعه املاک و پروژه‌های عمرانی",
        "معماری، طراحی داخلی و طراحی فضا",
        "تاسیسات، تعمیرات و نگهداری ساختمان",
        "خدمات فنی، برق، مکانیک و تاسیسات ساختمانی",
        "نظافت، خدمات ساختمانی و نگهداری اماکن",
        "تولید، کارخانه و صنایع تولیدی",
        "ماشین‌آلات و تجهیزات صنعتی",
        "ابزارآلات و تجهیزات فنی",
        "اتوماسیون صنعتی و کنترل",
        "مواد اولیه، قطعات و ملزومات تولید",
        "بسته‌بندی صنعتی و خدمات وابسته",
        "چاپ صنعتی و تولید سفارشی",
        "بازرسی فنی، کنترل کیفیت و خدمات مهندسی",
        "انرژی، نفت، گاز و پتروشیمی",
        "برق، آب، گاز و خدمات شهری",
        "انرژی‌های تجدیدپذیر و فناوری پاک",
        "محیط‌زیست، پایداری و مدیریت سبز",
        "بازیافت، پسماند و مدیریت منابع",
        "کشاورزی، زراعت و باغداری",
        "دامپروری، طیور و شیلات",
        "صنایع غذایی اولیه و فرآوری کشاورزی",
        "فناوری کشاورزی و تجهیزات کشاورزی",
        "خودرو، قطعات و لوازم یدکی",
        "خدمات خودرویی، تعمیر، سرویس و نگهداری",
        "موتورسیکلت، دوچرخه و حمل‌ونقل سبک",
        "حمل‌ونقل مسافر",
        "حمل‌ونقل کالا و باربری",
        "لجستیک، انبارداری و زنجیره تامین",
        "خدمات پیک، ارسال و تحویل کالا",
        "ورزش، تفریح و سرگرمی حضوری",
        "باشگاه‌ها، مراکز ورزشی و آکادمی‌ها",
        "گردشگری تفریحی و تجربه‌محور",
        "خدمات شخصی و سبک زندگی",
        "خدمات محلی و خدمات در محل",
        "خشکشویی، نظافت و خدمات خانگی",
        "امنیت، حفاظت و سیستم‌های ایمنی",
        "دولت و نهادهای عمومی",
        "سازمان‌های غیرانتفاعی، خیریه و اجتماعی",
        "انجمن‌ها، اتحادیه‌ها و نهادهای صنفی",
        "خدمات فرهنگی، اجتماعی و جامعه‌محور",
      ],
      showIf: {
        parentQuestion:
          "آیا قصد ورود به حوزه یا حوزه‌های جدید کسب‌وکار را دارید؟",
        equals: "بله",
      },
    },
    {
      id: 46,
      backendKey: "new_sales_methods",

      question:
        "آیا قصد دارید از روش جدیدی برای فروش محصول یا خدمات خود استفاده کنید؟",
      type: "radio",
      options: ["بله", "خیر"],
    },
    {
      id: 47,
      backendKey: "want_new_sales_methods",

      question: "روش‌های فروش مدنظر خود را مشخص کنید.",
      type: "checkbox",
      options: ["آنلاین", "آفلاین", "سایر"],
      otherOption: "سایر",
      otherPlaceholder: "موضوع مورد نظر خود را بنویسید",
      showIf: {
        parentQuestion:
          "آیا قصد دارید از روش جدیدی برای فروش محصول یا خدمات خود استفاده کنید؟",
        equals: "بله",
      },
    },
    {
      id: 48,
      backendKey: "new_payment_methods",

      question: "آیا قصد دارید از روش جدیدی برای پرداخت استفاده کنید؟",
      type: "radio",
      options: ["بله", "خیر"],
    },
    {
      id: 49,
      backendKey: "want_new_payment_methods",

      question: "روش‌های پرداخت مدنظر خود را مشخص کنید.",
      type: "checkbox",
      options: [
        "درگاه پرداخت داخلی",
        "درگاه پرداخت خارجی",
        "روش های پرداخت اقساطی و تسهیلات",
        "پرداخت درب منزل",
        "کیف پول",
      ],
      showIf: {
        parentQuestion: "آیا قصد دارید از روش جدیدی برای پرداخت استفاده کنید؟",
        equals: "بله",
      },
      checkboxSubOptions: {
        parentOption: "روش های پرداخت اقساطی و تسهیلات",
        label: "روش‌های پرداخت اقساطی و تسهیلات",
        options: [
          "اسنپ پی",
          "دیجی پی",
          "تارا",
          "اوانو",
          "ترب پی",
          "سلام پی",
          "ازکی پی",
          "تکنوپی",
          "کیپا",
          "دیجی شهر",
          "جی اس ام پی",
          "بالون",
          "دارا",
          "ستاره یک",
          "نسیبا",
        ],
      },
    },
    {
      id: 50,
      backendKey: "new_revenue_model",

      question: "آیا قصد اضافه کردن مدل درآمدی جدید دارید؟",
      type: "radio",
      options: ["بله", "خیر"],
    },
    {
      id: 51,
      backendKey: "want_new_revenue_model",

      question: "مدل یا مدل‌های درآمدی مدنظر خود را انتخاب کنید.",
      type: "checkbox",
      options: [
        "فروش محصول (Product Sales)",
        "دریافت هزینه خدمات (Service Fees)",
        "اشتراک دوره‌ای، ماهانه یا سالانه (Subscription)",
        "کمیسیون از معامله (Commission)",
        "کارمزد تراکنش (Transaction Fee)",
        "درآمد از تبلیغات (Advertising)",
        "نسخه رایگان + ارتقاء پولی (Freemium)",
        "لایسنس / حق استفاده (Licensing / Royalties)",
        "اجاره یا لیزینگ (Rental / Leasing)",
        "درآمد از همکاری در فروش (Affiliate / Referral Fees)",
        "درآمدزایی از داده، گزارش، بینش آماری یا API داده (Data Monetization)",
        "اسپانسرشیپ (Sponsorship)",
        "پرداخت بر اساس مصرف (Usage / Pay-as-you-go)",
        "کارمزد واسطه‌گری (Brokerage Fees)",
        "حق عضویت (Membership Fees)",
        "هزینه راه‌اندازی، نصب یا پیاده‌سازی (Setup / Implementation Fee)",
        "هزینه نگهداری و پشتیبانی (Maintenance / Support Fees)",
        "فروش بلیت / هزینه ورودی (Ticketing / Admission Fees)",
        "کمک مالی / گرنت / حمایت بلاعوض (Donations / Grants)",
        "درآمد بهره، سود تسهیلات یا تامین مالی (Interest / Financing Income)",
        "حق فرانچایز / اعطای نمایندگی (Franchise Fees)",
      ],
      showIf: {
        parentQuestion: "آیا قصد اضافه کردن مدل درآمدی جدید دارید؟",
        equals: "بله",
      },
    },
  ],
};
